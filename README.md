...
| **Reliability, measured** | `autoagent.eval.run_k()` reports `pass^k`, not just `pass@1` — because `pass^k ≈ p^k`, a 90 % agent is a 57 % agent over 8 tasks. The judge is a **deterministic host predicate**, never an LLM: on agent failures, LLM judges cap under 55 % accuracy |
